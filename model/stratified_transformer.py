...
def get_indice_pairs(
    ...
    """
    Params: p2v_map: [C_num, K] already batch splitted
        counts: [C_num, ]
        downsample_idx: [downsample_N,] index of downsampled points
        new_p2v_map: big window [C_num_big, K]
    """
    n, k = p2v_map.shape
    mask = torch.arange(k).unsqueeze(0).cuda() < counts.unsqueeze(-1)  # [n, k]
    mask_mat = mask.unsqueeze(-1) & mask.unsqueeze(
        -2
    )  # [n, k, k] valid attention range

    index_0 = p2v_map.unsqueeze(-1).expand(-1, -1, k)[
        mask_mat
    ]  # [M, ] for colum
    index_1 = p2v_map.unsqueeze(1).expand(-1, k, -1)[mask_mat]  # [M, ] for row
    ...
    mask = torch.arange(k).unsqueeze(0).cuda() < new_counts.unsqueeze(
        -1
    )  # [n, k]
    ...
    if i % 2 == 0:
        ...
        window_coord = torch.div(
            (xyz[new_p2v_map] - xyz_min), window_size, rounding_mode="trunc"
        )  # [n, k, 3]
    else:
    ...
    try:
        new_index_0 = new_p2v_map.unsqueeze(-1).expand(-1, -1, k)[
            mask_mat
        ]  # [M, ]
        new_index_1 = new_p2v_map.unsqueeze(1).expand(-1, k, -1)[
            mask_mat
        ]  # [M, ]
    except:
        if logger is not None:
            logger.info(
                "new window indices pairs too large... fall back to single window version"
            )
            logger.info(
                f"new_p2v_map.shape: {new_p2v_map.shape}"
            )  # new_p2v_map.shape: torch.Size([870, 1569])
            logger.info(
                f"mask_mat.shape: {mask_mat.shape}"
            )  # mask_mat.shape: torch.Size([870, 1569, 1569])
...
def grid_sample(pos, batch, size, start, return_p2v=True):
    """
    Params: pos: float [N, 3]
        batch: long [N]
        size: float [3, ]
        start: float [3, ] / None
    Return: cluster [N], p2v_map [C_num, K], counts [C_num]
    """
    ...
    # obtain p2v_map
    n = unique.shape[0]  # cluster num
    k = counts.max().item()
    p2v_map = cluster.new_zeros(n, k)  # [n, k]
    mask = torch.arange(k).cuda().unsqueeze(0) < counts.unsqueeze(-1)  # [n, k]
    p2v_map[mask] = torch.argsort(
        cluster
    )  # use 0 to represent no points in a cluster, contains index of points in a cluster
...
class TransitionDown(nn.Module):
    def __init__(
        self, in_channels, out_channels, ratio, k, norm_layer=nn.LayerNorm
    ):
        super().__init__()
        self.ratio = ratio
        self.k = k
    ...
    def forward(self, feats, xyz, offset, gt, query_base_y):
        ...
        for i in range(1, offset.shape[0]):
            count += (
                (offset[i].item() - offset[i - 1].item()) * self.ratio
            ) + 1
        ...
        return feats, n_xyz, n_offset, n_gt, n_query_base_y
...
class WindowAttention(nn.Module):
    ...
    def forward(self, feats, xyz, index_0, index_1, index_0_offsets, n_max):
        ...
        # attn_flat = pointops.attention_step1(query.float(), key.float(), index_1.int(), index_0_offsets.int())
        ...
        assert (
            relative_position_index <= 2 * self.quant_grid_length - 1
        ).all()
...
class BasicLayer(nn.Module):
    ...
    def forward(self, feats, xyz, offset, gt, query_base_y):
        ...
        # xyz: N, 3
        window_size = (
            torch.tensor([self.window_size] * 3).type_as(xyz).to(xyz.device)
        )

        offset_ = offset.clone()
        offset_[1:] = offset_[1:] - offset_[:-1]  # offset difference
        batch = (
            torch.cat(
        ...
        v2p_map, p2v_map, counts = grid_sample(
            xyz, batch, window_size, start=None
        )
        ...
        # get new_offset after fps for sparse window sampling
        ...
        new_window_size = 2 * torch.tensor([self.window_size] * 3).type_as(
            xyz
        ).to(xyz.device)
        ...
        if self.downsample:
            feats_down, xyz_down, offset_down, gt_down, query_base_y_down = (
                self.downsample(feats, xyz, offset, gt, query_base_y)
            )
        else:
            feats_down, xyz_down, offset_down, query_base_y_down = (
                None,
                None,
                None,
                None,
...
class Stratified(nn.Module):
    def __init__(
        ...
        if stem_transformer:
            ...
            self.downsample = TransitionDown(
                channels[0], channels[1], ratio, k
            )
            self.layer_start = 1
        ...
        self.layers = nn.ModuleList(
            [
                BasicLayer(
                    ...
                    k=k,
                    out_channels=(
                        channels[i + 1] if i < num_layers - 1 else None
                    ),
                    logger=logger,
                )
    ...
    def forward(
        self, feats, xyz, offset, batch, neighbor_idx, gt, query_base_y=None
    ):
        """
        Params:
            feats:N,6 xyz: N,3
            offset: b batch: N (0..0, 1..1,...,b)
            neighbor_idx: N,34 no_neigh --> -1
        """
        feats_stack = []
        xyz_stack = []
        offset_stack = []
        ...
        for i, layer in enumerate(self.layers):
            (
                ...
                gt_down,
                query_base_y_down,
            ) = layer(feats, xyz, offset, gt, query_base_y)

            feats_stack.append(feats)
            xyz_stack.append(xyz)
            offset_stack.append(offset)
            gt_stack.append(gt)
            query_base_y_stack.append(query_base_y)

            feats = feats_down
            xyz = xyz_down
            offset = offset_down
            gt = gt_down
            query_base_y = query_base_y_down

        if self.backbone:
            # when used as backbone
            out = self.classifier(feats_stack[-1])
            upsample_out = pointops.interpolation(
                xyz_stack[-1],
                xyz_stack[-2],
                out,
                offset_stack[-1],
            ...
            return out, gt_stack[-1], upsample_out
