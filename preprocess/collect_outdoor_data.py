...
def map_street_sign_class(data):
    data[:, 6] = np.where(data[:, 6] == 15, data[:, 7], data[:, 6] + 15)
...
if __name__ == "__main__":
    ...
    for txt_file in txt_files:
        ...
        file_name = f"{txt_file.split('S')[0].trim()}_{txt_file.split('part')[-1].split('.')[0] if 'part' in txt_file else ''}_processed.npy".replace(" ", "")
