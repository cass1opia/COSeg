...
class Outdoor_base(Dataset):
    def __init__(
        ...
        if cvfold == 0:
            self.test_classes = [self.type2class[i] for i in self.fold_0 if i in self.type2class]
        elif cvfold == 1:
            self.test_classes = [self.type2class[i] for i in self.fold_1 if i in self.type2class]
        else:
...
class Outdoor(Outdoor_base):
    """
    The Outdoor dataset class used for backbone pretraining.
    """
    ...
    def __init__(
        ...
        print(
            "[Outdoor Pretrain Dataset] Mode: {0} | Num_blocks: {1}".format(
...
class Outdoor_FS(Outdoor_base):
    """
    The Outdoor dataset class used for few-shot learning.
    """
...
class Outdoor_FSForVIS(Outdoor_FS):
    """
    The Outdoor dataset class used for visualizing the few-shot results.
    """
    ...
    def crop_point_cloud(self, point_cloud, feat, labels):
        ...
        # Choose the quadrant with the most points with label 1
        counts = [q1_count, q2_count, q3_count, q4_count]
        max_count_index = np.argmax(counts)

        if max_count_index == 0:  # q1: left half
            mask = point_cloud[:, 0] <= half_x
        elif max_count_index == 1:  # q2: right half
            mask = point_cloud[:, 0] > half_x
        elif max_count_index == 2:  # q3: top half
            mask = point_cloud[:, 1] > half_y
        else:  # q4: bottom half
            mask = point_cloud[:, 1] <= half_y

        return point_cloud[mask], feat[mask], labels[mask]
    ...
    def __init__(
        ...
        if split == "val":
            self.test_data_path = os.path.join(
                ...
                "Outdoor_S_%d_N_%d_K_%d_episodes_%d_pts_%d_vs_%.2f_rdmsp"
                % (
                    cvfold,
                    n_way,
                    k_shot,
            ...
            self.test_data_path = os.path.join(
                os.path.dirname(data_root),
                "Outdoor_S_%d_N_%d_K_%d_test_episodes_%d_pts_%d_vs_%.2f"
                % (
                    cvfold,
                    n_way,
                    k_shot,
