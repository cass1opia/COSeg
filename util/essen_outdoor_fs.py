...
class ESSEN_OUTDOOR_base(Dataset):
    def __init__(
        self,
        split="train",
        data_root="trainval",
        ...
        self.class_count = 10  # Update based on your actual number of classes
        class_names = open(
            os.path.join(
                os.path.dirname(os.path.dirname(data_root)),
                "meta",
                "essen_outdoor_classnames.txt",
            )
        ).readlines()
        self.class2type = {
            i: name.strip() for i, name in enumerate(class_names)
        }
        print("ESSEN_OUTDOOR Classes:", self.class2type)
        self.type2class = {self.class2type[t]: t for t in self.class2type}
        
        # Define cross-validation folds for ESSEN_OUTDOOR dataset
        # Fold 0: Classes for few-shot testing when cvfold=0
        self.fold_0 = [
            "vehicle",
            "traffic_sign", 
            "vegetation",
        ...
        # Fold 1: Classes for few-shot testing when cvfold=1
        self.fold_1 = [
            "road",
            "sidewalk", 
            "person",
        ...
        all_classes = [i for i in range(0, self.class_count)]
        self.train_classes = [
    ...
    def get_class2scans(self):
        ...
        if os.path.exists(class2scans_file):
            ...
            print("==== ESSEN_OUTDOOR class to scans mapping is done ====")
...
class ESSEN_OUTDOOR(ESSEN_OUTDOOR_base):
    """
    The ESSEN_OUTDOOR dataset class used for backbone pretraining.
    """
    ...
    def __init__(
        ...
        print(
            "[ESSEN_OUTDOOR Pretrain Dataset] Mode: {0} | Num_blocks: {1}".format(
...
class ESSEN_OUTDOOR_FS(ESSEN_OUTDOOR_base):
    """
    The ESSEN_OUTDOOR dataset class used for few-shot learning.
    """
    ...
    def __init__(
        ...
        print("ESSEN_OUTDOOR Classes: {0} in {1} set".format(self.classes, split))
    ...
    def sample_pointcloud(
        ...
        feat = torch.cat((coord, feat), dim=1)
        if support:
            # Create a new label tensor for base classes
...
class ESSEN_OUTDOOR_FSForVIS(ESSEN_OUTDOOR_FS):
    """
    The ESSEN_OUTDOOR dataset class used for visualizing the few-shot results.
    """
...
class ESSEN_OUTDOOR_FS_TEST(Dataset):
    def __init__(
        self,
        split="val",
        data_root="trainval",
        ...
        if split == "val":
            self.test_data_path = os.path.join(
                ...
                "ESSEN_OUTDOOR_S_%d_N_%d_K_%d_episodes_%d_pts_%d_vs_%.2f_rdmsp"
                % (
                    cvfold,
                    n_way,
                    k_shot,
            ...
            self.test_data_path = os.path.join(
                os.path.dirname(data_root),
                "ESSEN_OUTDOOR_S_%d_N_%d_K_%d_test_episodes_%d_pts_%d_vs_%.2f"
                % (
                    cvfold,
                    n_way,
                    k_shot,
    ...
    def prepare_test_data(self):
        if os.path.exists(self.test_data_path):
            ...
            print(
                f"ESSEN_OUTDOOR Test dataset ({self.test_data_path}) does not exist...\n Constructing..."
            )
